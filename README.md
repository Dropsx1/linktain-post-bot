...
`POST_CHAT_ID` defaults to [`@Dropsnsfw`](https://t.me/Dropsnsfw), so posts go
there unless you override the variable. Setting it to a blank value makes the
...
| POST_CHAT_ID | publish targets, comma-separated (`@name`, `-100…`, or `-100…:<topicId>` for a forum topic); default `@Dropsnsfw`, blank = preview only |
