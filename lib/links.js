...
// A t.me permalink for a channel post, so the operator can jump straight to it.
// Public channels are addressed by @name, private ones by -100<internal id>.
function channelPostLink(chatId, messageId) {
  const id = String(chatId || '').trim();
  if (!id || messageId == null) return null;
  if (id.startsWith('@')) return `https://t.me/${id.slice(1)}/${messageId}`;
  const numeric = id.match(/^-100(\d+)$/);
  if (numeric) return `https://t.me/c/${numeric[1]}/${messageId}`;
  return null;
}
