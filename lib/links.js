...
// Telegram rejects the entire keyboard if any url button is malformed, so a bad
// env value should drop one button rather than break every /start.
function buttonSpecs(config) {
  return [
    ['Access Tutorial', tutorialLink(config)],
    ['VIP Access', config.vipUrl],
    ['Discord', config.discordUrl],
  ]
    .map(([label, value]) => ({ label, url: httpUrl(value) }))
